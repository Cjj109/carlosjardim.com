/**
 * Correo, pérdida del aparato y recuperación.
...
await alta(samsung, inv1.enlace, 'carlos@ejemplo.com', 'Samsung');
await samsung.pag.waitForURL(/\/calculadora/, { timeout: 15000 });
ok('entra tras darse de alta con su correo', (await samsung.ctx.request.get(`${BASE}/api/bcv`)).status(), 200);

console.log('\nSIN CORREO NO SE PUEDE DAR DE ALTA');
const inv2 = await (await fetch(`${BASE}/api/acceso/invitar`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ para: 'SinCorreo', maestra: MAESTRA }) })).json();
const otro = await aparato();
await alta(otro, inv2.enlace, '', 'Sin correo');
await otro.pag.waitForTimeout(2500);
ok('se le pide un correo válido', (await otro.pag.textContent('#mensaje')).includes('correo'), true);

console.log('\nENTRAR ESCRIBIENDO EL CORREO (el camino de repuesto)');
const conCorreo = await (await fetch(`${BASE}/api/acceso/reto?tipo=entrada&correo=carlos@ejemplo.com`)).json();
ok('devuelve la llave de esa persona', conCorreo.llaves.length, 1);
const desconocido = await (await fetch(`${BASE}/api/acceso/reto?tipo=entrada&correo=nadie@ejemplo.com`)).json();
ok('un correo desconocido no delata nada', { ok: desconocido.ok, llaves: desconocido.llaves.length }, { ok: true, llaves: 0 });
...
await alta(perdido, rescate.enlace, 'carlos2@ejemplo.com', 'Teléfono nuevo');
