...
function initSnake() {
  ...
  console.log('Tile size:', snake.tileSize);
...
function gameOver() {
  snake.running = false;
  setTimeout(() => {
    alert(`Game Over! Puntos: ${snake.score}`);
  }, 100);
}
...
function drawSnake() {
  ...
  // Obtener color accent
  const styles = getComputedStyle(document.documentElement);
  const ar = styles.getPropertyValue('--accent-r').trim() || '255';
  const ag = styles.getPropertyValue('--accent-g').trim() || '59';
  const ab = styles.getPropertyValue('--accent-b').trim() || '48';
  ...
  ctx.fillText(`Food: ${snake.food.x},${snake.food.y}`, 5, 15);
  ctx.fillText(`Head: ${snake.snake[0].x},${snake.snake[0].y}`, 5, 30);
  ctx.fillText(`Length: ${snake.snake.length}`, 5, 45);
}
